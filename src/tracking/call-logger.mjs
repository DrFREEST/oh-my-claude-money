/**
 * call-logger.mjs - MCP 호출 세션별 로깅
 *
 * fusion-router에서 MCP(ask_codex/ask_gemini)로 라우팅 시 호출 정보를 세션별로 기록
 * JSONL 형식으로 효율적 저장
 */
 ...
 * @param {number} [callData.inputTokens] - 실제 입력 토큰
 * @param {number} [callData.outputTokens] - 실제 출력 토큰
 * @param {number} [callData.reasoningTokens] - 추론 토큰
 * @param {number} [callData.cost] - 비용
 ...
 */
export function logMcpCall(sessionId, callData) {
  if (!sessionId) return;
  ...
  const logFile = join(sessionDir, 'mcp-calls.jsonl');
  const entry = {
    timestamp: new Date().toISOString(),
    provider: callData.provider || 'unknown',
    model: callData.model || '',
    agent: callData.agent || '',
    // 실제 토큰 데이터 (MCP 응답에서 획득, 우선)
...
export function getSessionCalls(sessionId, since) {
  ...
  // Support both new mcp-calls.jsonl and legacy opencode-calls.jsonl
  let logFile = join(SESSIONS_DIR, sessionId, 'mcp-calls.jsonl');
  if (!existsSync(logFile)) {
    logFile = join(SESSIONS_DIR, sessionId, 'opencode-calls.jsonl');
  }
...
export const logOpenCodeCall = logMcpCall;
