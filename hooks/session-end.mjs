#!/usr/bin/env node
/**
 * session-end.mjs - SessionEnd 이벤트 안전 패스스루
 *
 * 세션 종료 이벤트를 최소 로깅/상태 저장한 뒤 항상 continue 처리합니다.
 */
...
function getNumber(input, keys) {
  ...
  while (index < keys.length) {
    var key = keys[index];
    if (input && typeof input[key] === 'number') {
      return input[key];
    }
    index += 1;
  }
...
function getReason(input) {
  ...
  if (input && typeof input.end_reason === 'string') {
    return input.end_reason;
  }
  if (input && typeof input.endReason === 'string') {
    return input.endReason;
  }
  return '';
}
...
function buildEvent(input) {
  return {
    ...
    durationMs: getNumber(input, ['duration_ms', 'durationMs']),
    turns: getNumber(input, ['turn_count', 'turnCount', 'total_turns'])
  };
}
